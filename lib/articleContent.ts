...
import type { ArticleContent, ArticleMetric, NewsArticle } from "@/types";
...
function extractFirst(pattern: RegExp, html: string): string {
...
function htmlToText(html: string): string {
  ...
  const lines = normalizeWhitespace(stripTags(blockSeparated))
    ...
    .filter((line) => {
      ...
      if (lower.includes("subscribe") && line.length < 120) return false;
      if (lower.includes("sign in") && line.length < 120) return false;
...
function metricContext(text: string, index: number, length: number): string {
...
export function extractMetricCandidates(text: string): ArticleMetric[] {
  const patterns: Array<{ kind: ArticleMetric["kind"]; pattern: RegExp }> = [
  ...
  for (const { kind, pattern } of patterns) {
    for (const match of text.matchAll(pattern)) {
      ...
      metrics.push({
        ...
        context: metricContext(text, match.index ?? 0, rawText.length),
      });
...
export function extractArticleContent(html: string, url: string, articleId = ""): ArticleContent {
  ...
  const title =
    extractFirst(/<article\b[\s\S]*?<h1[^>]*>([\s\S]*?)<\/h1>/i, cleanHtml) ||
...
export async function fetchArticleContent(article: NewsArticle): Promise<ArticleContent> {
  if (!article.link) {
    return {
...
export function attachContentToArticle(article: NewsArticle, content: ArticleContent): NewsArticle {
  ...
  return {
    ...article,
    description: article.description || content.excerpt,
